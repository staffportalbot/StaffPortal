		...
		return new winston.Logger({
			...
			transports: [
				...
				new winston.transports.DailyRotateFile({
					...
					filename: require("path").join(process.cwd(), `logs/${type === "master" ? "master" : type.replace(/ /g, "-")}-staffportal.log`),
				}),
				new winston.transports.DailyRotateFile({
					name: "Full Output",
					level: config.fileLevel,
					...
					filename: require("path").join(process.cwd(), `logs/staffportal.log`),
				}),
				new winston.transports.File({
					level: config.consoleLevel,
					json: true,
					colorize: false,
					filename: require("path").join(process.cwd(), `logs/verbose.staffportal.log`),
				}),
			],
		});
	}
